...
## Featured Node in this Release

*(Coming soon — the upcoming release will feature the **JLC ControlNet Orchestrator**, a unified ControlNet execution node that integrates ControlNet loading, conditioning, and fusion into one cohesive pipeline. Designed for improved performance, deterministic behavior, and scalable multi-ControlNet workflows.)*
...
- Full alignment with expected ControlNet semantics
- Establishes ControlNet Composition as a **mathematically consistent, order-invariant fusion operator** when `alpha = 1`  
...
A correctness-focused ControlNet fusion node with improved performance over standard recursive chaining, replacing recursive execution with **explicit parallel evaluation**:
...
PNG workflows contain the embedded ComfyUI graph and can be dragged directly into the ComfyUI canvas. The PNG with embedded workflow generated by ComfyUI may appear broken, but opens correctly.
